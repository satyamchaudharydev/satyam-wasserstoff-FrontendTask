...
import { Note, NoteFileContent, useFileContentContext } from '@/store/FileContentContext';
...
const NoteMaker = ({ content }: { content: NoteFileContent}) => {
  ...
  return (
    <div className="flex space-x-4 p-6 flex-col gap-8">
      <Button className="h-fit mr-auto"
...
const NoteEditor = ({onClose, onSave, selectedNote }: 
    { 
      onClose: () => void, 
  ...
  return (
    <div className="p-8 w-full h-full absolute inset-0 z-[9999] backdrop-blur-sm">
      <div className='h-full bg-background rounded px-3 flex flex-col'>
        <div className='min-h-[30px] border-b border-[#393b3f] p-2 flex justify-between'>
          <div className='flex justify-center items-center gap-2'>
            <Button className='text-[14px] p-1 px-2' onClick={() => {
        ...
        <textarea
          className="w-fullmt-3 p-2 flex-1  bg-transparent  border-white focus:outline-none focus:ring-2 focus:ring-primary-light"
          value={noteData}
