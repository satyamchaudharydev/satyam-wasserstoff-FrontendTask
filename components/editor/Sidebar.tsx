...
const Sidebar: React.FC = () => {
  ...
  const {fileSystem,selectedItem,setFileSystem,setSelectedItem, updateOpenFiles} = useContext(EditorContext);
    ...
    if (selectedItem === null) {
      ...
      for (let i = 0; i < path.length; i++) {
        if (i === path.length - 1) {
          ...
          current[path[i]].children = { ...current[path[i]].children, ...newFile };
        } else {
          current = current[path[i]].children;
        }
      }
      console.log(newFileSystem, "newFileSystem added")
  ...
  const onAddFolder = () => {
    ...
    if(selectedItem === null) {
      ...
      for (let i = 0; i < path.length; i++) {
        if (i === path.length - 1) {
          current[path[i]].children = { ...current[path[i]].children, ...newFolder };
        } else {
          current = current[path[i]].children;
        }
      }
  ...
  const onTitleChange = (newTitle: string, filePath: string) => {
  ...
  const renderFileSystem = (obj: FileSystem, path: string[] = []): JSX.Element[] => {
    const sortedEntries = Object.entries(obj).sort(([keyA, valueA], [keyB, valueB]) => {
    ...
    return sortedEntries.map(([key, value]) => {
      ...
      if (value.type === 'folder') {
        return (
          <div key={itemPath} className="ml-1 relative">
            <motion.div 
              className={cn("flex items-center cursor-pointer p-1 rounded-md relative z-10 isolate", isSelected && 'bg-gray')}
              onClick={() => {
            ...
            >
              ...
              <FileTitle 
                  ...
                  onTitleChange={(newTitle, filePath) => onTitleChange(newTitle, filePath)} 
                  icon={value.isOpen ? '📂' : '📁'} 
        ...
        return (
          console.log(itemPath,key, "itemPath"),
          <div key={itemPath} className="ml-1 relative cursor-pointer">
            <motion.div 
              className={cn("flex items-center p-1 rounded-md relative z-10", isSelected && 'bg-gray')}
              onClick={() => {
                setSelectedItem({
                  type: "file",
                  name: itemPath
  ...
  return (
    <div className=" bg-foreground h-screen p-4 overflow-auto" onClick={handleSidebarClick}>
      <SideBarHeader 
        onAddFile={onAddFile} 
